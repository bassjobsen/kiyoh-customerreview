...
function kiyoh_getOption()
{
    ...
    $kiyoh_options['productreviews'] 		= get_option('kiyoh_option_productreviews');
...
function kiyoh_sendMail($options)
{
	...
	$email 			= $options['email'];
    $ean_numbers    = $options['ean_numbers'];
    ...
    if($kiyoh_options['send_method']=='kiyoh'){
        ...
        if($kiyoh_options['productreviews'] == 'Yes') {
          foreach($ean_numbers as $ean_number) {
            $url .= '&p_r%5B%5D='.rawurlencode($ean_number); // encode sku
          }  
        }
        //echo $url; exit;
        ...
        if($kiyoh_options['productreviews'] == 'Yes') {
          ...
          foreach($ean_numbers as $ean_number) {
            $link .= ($i===0) ? '?' : '&';
            $link .= 'p_r%5B%5D='.rawurlencode($ean_number); // encode sku
            $i++;
          }    
        }
